<template>
  ...
  <div class="app-container" style="padding-top: 45px; padding-left: 0px;">
    <div class="filter-container">
      ...
      <router-link :to="'/setting/user/add'">
        <el-button
    ...
    <el-table v-loading="dialogPermissionLoading" :data="usersList1" stripe>
      ...
      <el-table-column :label="$t('role')" sortable width="180">
        <template slot-scope="{row}">
          <p v-for="role in row.roles" :key="role.id">{{ role.name }}</p>
        </template>
      </el-table-column>
      <el-table-column prop="email" :label="$t('email')" sortable width="230" />
      <el-table-column :label="$t('focus')" sortable width="140">
        <template slot-scope="{row}">
          <p v-for="role in row.roles" :key="role.id">{{ role.focus }}</p>
        </template>
      </el-table-column>
      <el-table-column :label="$t('status')" sortable width="110">
        <template slot-scope="{row}">
        ...
        <template slot-scope="{row}" style="display: initial;">
          <el-dropdown class="avatar-container right-menu-item hover-effect" trigger="click">
            ...
            <el-dropdown-menu slot="dropdown">
              <router-link :to="'/setting/user/edit/'+row.id">
                <el-dropdown-item v-if="checkPermission('Add_user')">{{ $t('edit') }}</el-dropdown-item>
              </router-link>
    ...
    <el-dialog :visible.sync="dialogPermissionVisible" :title=" $t('edit_permissions')+' - ' + currentUser.name">
      <div v-loading="dialogPermissionLoading" class="form-container">
        <div class="permissions-container" style="height: 58vh; width: 100%; overflow-y: scroll;">
          <div class="block" style="display: flex;">
            ...
            <el-form label-width="80px" label-position="top" style="margin-left: 130px;">
...
export default {
  ...
  methods: {
    ...
    getAllUsers(page) {
      ...
      this.axios
        .post(store.getters.api_route + 'accounting/users/index?page=' + page, formdata, config)
        .then(res => {
          const { data } = res
          if (!data) {
            console.log('sdfsf')
    ...
    handleFilter() {
      ...
      this.axios
        .post(store.getters.api_route + 'accounting/users/getallusers', formdata, config)
        .then(res => {
          const { data } = res
          if (!data) {
            console.log('sdfsf')
    ...
    handleModifyStatus(row, status) {
      ...
      formdata.append('token', store.getters.token)
      this.axios
        .post(store.getters.api_route + 'accounting/user/change_status', formdata, config)
        .then(res => {
          this.$message({
    ...
    getallpermissions() {
      ...
      formdata.append('token', store.getters.token)
      this.axios
        .post(store.getters.api_route + 'accounting/permissions/index', formdata, config)
        .then(res => {
          const { data } = res
          if (!data) {
            console.log('sdfsf')
    ...
    handleModifypermissions(id) {
      ...
      formdata.append('token', store.getters.token)
      this.axios
        .post(store.getters.api_route + 'accounting/user/change_permissions', formdata, config)
        .then(res => {
