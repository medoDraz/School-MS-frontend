        ...
        <el-col :span="12">
          <div class="grid-content bg-purple">
            <el-container>
              ...
              >{{ $t('DeliveredOrders') }}</el-header>

              <div style="height: 680px;overflow: auto;">
                <div
                  v-for="transition of delivered_items"
                  :key="transition.id"
                  class="el-card card-item"
                  style="margin:5px 0px;  background:rgb(225, 243, 216)"
                  @click="getDetails(transition)"
                >
                  <el-button type="text" style="width: 100%;">
                    <div class="text item" style="padding:5px 20px;">
...
export default {
  ...
  methods: {
    getBreaks() {
      ...
      this.axios.post(store.getters.api_route + 'pages/breaks', formdata, config)
        .then(res => {
          // console.log(res.data.breaks)
          const { data } = res
          if (!data) {
            console.log('sdfsf')
          }
          ...
          current_time.setHours(moment().format('HH:mm').split(':')[0], moment().format('HH:mm').split(':')[1])
          const break2 = new Date()
          break2.setHours(res.data.breaks.break2.split(':')[0], res.data.breaks.break2.split(':')[1])

          if (current_time >= break2) {
            console.log(res.data.breaks.break2)
            this.breaks = '2'
          } else {
            console.log(res.data.breaks.break1)
            this.breaks = '1'
          }
          this.getAllItems()
        })
